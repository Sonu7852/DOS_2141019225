...
public class P1 {
 ...
 wt[0] = 0; 
 ct[0] = burstTimes[0]; 
 tat[0] = ct[0] - arrivalTimes[0]; // Corrected turnaround time calculation 
 for (int i = 1; i < n; i++) { 
 ...
 System.out.println("\nProcesses || Burst Time || Arrival Time || Waiting Time || Response Time || Turnaround Time || Completion Time "); 
 float awt = 0; 
 float art = 0; 
 float att = 0; 
 for (int i = 0; i < n; i++) { 
...
arrivalTimes)); 
	...
	private static void findWaitingTime(int processes[], int n, int burstTimes[], int wt[], int quantum,
			int arrivalTimes[], int ct[], int rt[]) {
	...
	private static void compareAlgorithmsEfficiency(double avgWaitingTimeRR, double 
avgWaitingTimeFCFS) { 
 ...
 System.out.println("FCFS algorithm results in the minimum average waiting time = " + 
avgWaitingTimeFCFS); 
 } else { 
